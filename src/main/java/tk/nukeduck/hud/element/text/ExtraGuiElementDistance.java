...
public class ExtraGuiElementDistance extends ExtraGuiElementText {
	...
	public void render(Minecraft mc, ScaledResolution resolution, StringManager stringManager, LayoutManager layoutManager) {
		...
		Block te = mc.theWorld.getBlockState(mop.getBlockPos()).getBlock();
		if(te != null && !te.equals(Blocks.AIR)) {
			if(this.posMode.index == 0 && this.pos.value == Position.MIDDLE_CENTER) {
    			...
    			this.textBounds = new Bounds(xPos, yPos, textWidth, mc.fontRendererObj.FONT_HEIGHT);
    			mc.ingameGUI.drawString(mc.fontRendererObj, s, xPos, yPos, this.getColor());
			} else {
	...
	protected String[] getText(Minecraft mc) {
		RayTraceResult mop = mc.getRenderViewEntity().rayTrace(200, 1.0F);
		double x = (mop.getBlockPos().getX() + 0.5) - mc.thePlayer.posX;
		double y = (mop.getBlockPos().getY() + 0.5) - mc.thePlayer.posY;
		double z = (mop.getBlockPos().getZ() + 0.5) - mc.thePlayer.posZ;
		int distance = (int) Math.round(Math.sqrt(x*x + y*y + z*z));
		return new String[] {FormatUtil.translatePre("strings.distance." + mode.getValue(), String.valueOf(distance))};
	}
}