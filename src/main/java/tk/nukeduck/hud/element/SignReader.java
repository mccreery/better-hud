...
public class SignReader extends HudElement {
	...
	public SignReader() {
		...
		this.settings.add(type = new SettingMode("mode", new String[] {"sign.text", "sign.visual"}));
	}
		...
		if(te != null && te instanceof TileEntitySign) {
			if(type.getValue() == "sign.text") {
				...
				if(posMode.index == 1) {
					...
					pos2.update(event.getResolution(), new Bounds(0, 0, 88, 48));
					...
					FMLClientHandler.instance().getClient().renderEngine.bindTexture(signTex);
					glPushMatrix();
					glTranslatef(event.getResolution().getScaledWidth() - 93, (float) layoutManager.get(Position.TOP_RIGHT), 0F);
					glScalef(1, 0.5F, 1); // Texture is treated as square rather than a rectangle
					MC.ingameGUI.drawTexturedModalRect(0, 0, 16, 16, 88, 96); // UVs multiplied by 8 for some reason
					glPopMatrix();
					
					this.bounds = new Bounds(event.getResolution().getScaledWidth() - 93, layoutManager.get(Position.TOP_RIGHT), 88, 48);
					...
					if(te instanceof TileEntitySign) {
						for(ITextComponent ic : ((TileEntitySign) te).signText) {
							if(ic == null) ic = new TextComponentString("");
							String s = ic.getFormattedText();
							MC.fontRenderer.drawString(s, event.getResolution().getScaledWidth() - 49 - (MC.fontRenderer.getStringWidth(s) / 2), y, Colors.BLACK);
							//i++;
